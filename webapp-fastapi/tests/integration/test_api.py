...
class TestAPI:
    ...
    async def test_api_wind_default_params_with_data(
        self, integration_client_with_bulk_data
    ):
    ...
    async def test_api_wind_custom_station_with_data(
        ...
        response = await integration_client_with_bulk_data.get("/api/wind?stn=CYYZ")
        assert response.status_code == 200
    ...
    async def test_api_wind_time_range_with_data(
        ...
        response = await integration_client_with_bulk_data.get(
    ...
    @pytest.mark.anyio
    async def test_api_wind_data_consistency(
        ...
        for api_obs in api_data["winddata"]:
            ...
            matching_db_record = next(
                (obs for obs in db_wind_obs if obs["update_time"] == api_timestamp), None
            )
