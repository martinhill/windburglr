...
class WatchdogService:
    ...
    async def handle_scraper_status_update(self, scraper_status: ScraperStatus):
        """Handle scraper status update notifications."""
        try:
            logger.debug("Received scraper status update: %s", scraper_status)

            # Store the previous status to detect changes
            previous_status = self.get_station_status_by_name(
                scraper_status.station_name
            )

            # Update the internal status directly with the ScraperStatus object
            await self._update_station_status_with_model(scraper_status)

            # Check if status field changed or is new, and notify via websocket
            current_status = self.get_station_status_by_name(
                scraper_status.station_name
            )
            ...
            if previous_status is None:
                ...
                previous_status.status != current_status.status  or
                previous_status.retry_count != current_status.retry_count):
            ...
            if should_send_update and self.websocket_manager:
                await self.websocket_manager.send_station_status_update(
                    scraper_status.station_name, self
                )

            logger.info(
                "Processed scraper status update notification: %s", scraper_status
            )
